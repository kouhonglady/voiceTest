...
/**
 * 
 * 创建时间：创建时间：2018年4月27日 上午10:47:22 项目名称：voiceTest
 * 
 * @author lingxue
 * @version 1.0
 * @since JDK 1.8 文件名称：IndexManager.java
 * 类说明：Lucene检索管理类，包括索引的创建，创建索引的关键字段，创建索引的方式等等内容
 */

public class IndexManager {
	static String HOME_PATH = IndexManagerHelper.class.getResource("/..").getPath();

	private String indexPath = HOME_PATH +"/dataset/index"; // 索引路径
	...
	public Iterator<PCpair> search(String input, int start, int end) throws Exception {
		...
		for (int i = start; i < scoreDocs.length; i++) {
			...
			docs.add(new PCpair(d.getValues("id")[0], d.getValues("question")[0], d.getValues("answer")[0],
					scoreDocs[i].score));
		}
	...
	/**
	 * 
	 * PCpair 为 IndexManager类的内部类 用于检索过程中最小单元的描述，每个最小单元为检索得到一个问句对
	 * 包含四个属性，i(id)表示句子对的编号 q(question) 问句对中的问题 a(answer) 问句对中的答案 s(score) 检索得分
	 */
